...
import { MultiTouchCrosshairPaneView } from "./crosshair-line-pane";
import type { TooltipCrosshairLineData } from "./crosshair-line-pane";
import { DeltaTooltipPaneView } from "./delta-tooltip-pane";
import type {
	DeltaSingleTooltipData,
	DeltaTooltipData,
} from "./delta-tooltip-pane";
import { MultiTouchChartEvents } from "./multi-touch-chart-events";
import type { MultiTouchInteraction } from "./multi-touch-chart-events";
...
export class DeltaTooltipPrimitive implements ISeriesPrimitive<Time> {
	...
	constructor(options: Partial<TooltipPrimitiveOptions>) {
		...
		this._crosshairPaneView = new MultiTouchCrosshairPaneView(
			this._crosshairData,
		);
	...
	setData(
		crosshairData: TooltipCrosshairLineData[],
		tooltipData: Partial<DeltaTooltipData>,
	) {
	...
	private _showTooltip(interactions: MultiTouchInteraction) {
		...
		if (priceValues.length > 1) {
			...
			deltaContent.deltaBackgroundColor = positive
				? "rgb(4,153,129, 0.2)"
				: "rgb(239,83,80, 0.2)";
			deltaContent.deltaTextColor = positive
				? "rgb(4,153,129)"
				: "rgb(239,83,80)";
